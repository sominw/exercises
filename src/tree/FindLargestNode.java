...
public class FindLargestNode {
	...
	public static int findLargestNode(TreeNode<Integer> node) {
		if (node == null)
			return Integer.MIN_VALUE;
		
		int result = node.data;
		for (int i = 0; i < node.children.size(); i++) {
			int largestChild = findLargestNode(node.children.get(i));
			if (largestChild > result)
				result = largestChild;
		}
	...
	public static void main(String[] args) {
		...
		System.out.println("Largest node: " + findLargestNode(root));
	}
}
